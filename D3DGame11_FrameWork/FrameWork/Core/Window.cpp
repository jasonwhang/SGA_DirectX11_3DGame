...
void Window::Initialize()
{
	...
	// 화면이 중앙에 생성되도록 하는 작업이다.
	uint centerX
		= static_cast<uint>((GetSystemMetrics(SM_CXSCREEN) - Settings::Get().GetWidth()) * 0.5f);
	uint centerY
		= static_cast<uint>((GetSystemMetrics(SM_CYSCREEN) - Settings::Get().GetHeight()) * 0.5f);
